...
function GetItemIndex(itemnName, unitEntity)
	local itemIndex = 0
	local counter = 0
...
function StackItem(cd, max_stacks, modifier_name, ability, caster)
	if not ability.shortCD then
		...
		Timers:CreateTimer(cd, function()
			...
			if ability.current_stacks < max_stacks then
				...
				if ability.current_stacks < max_stacks then
					StackItem(cd, max_stacks, modifier_name, ability, caster)
				end
			end
			return nil
		end)
