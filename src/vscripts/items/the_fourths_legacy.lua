function ChannelTeleport(event)
	...
	local targetEntities = FindUnitsInRadius(caster:GetTeamNumber(), target_point, nil, FIND_UNITS_EVERYWHERE,
		DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_BUILDING,
	...
	ability:ApplyDataDrivenModifier(caster, caster, "modifier_scroll_of_town_portal_caster", {
		duration = teleport_delay
	})

	caster:EmitSound("Hero_KeeperOfTheLight.Recall.Cast")

	event.ability.particle_caster = ParticleManager:CreateParticle("particles/items2_fx/teleport_end.vpcf",
	...
	event.ability.particle_target = ParticleManager:CreateParticle("particles/items2_fx/teleport_end.vpcf",
