function placeWard(keys)
	...
	if tablelength(keys.ability.wards) > max_wards_placed then
...
function rechargeThirdEye(keys)
	...
	local distance_to_shop_for_recharge = ability:GetLevelSpecialValueFor("distance_to_shop_for_recharge",
		(ability:GetLevel() - 1))
