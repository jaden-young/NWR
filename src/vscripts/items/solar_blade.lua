function applyDamage(keys)
	...
	if keys.caster:IsRealHero() then
		...
		local targetEntities = FindUnitsInRadius(caster:GetTeamNumber(), caster:GetAbsOrigin(), nil, radius,
			DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0, FIND_ANY_ORDER, false)
		...
		if targetEntities then
			for _, target in pairs(targetEntities) do
				...
				local fire = ParticleManager:CreateParticle("particles/dire_fx/fire_barracks_glow_b.vpcf",
					PATTACH_ABSORIGIN_FOLLOW, keys.target)
				ParticleManager:SetParticleControlEnt(fire, 0, target, PATTACH_POINT_FOLLOW, "attach_hitloc",
					target:GetAbsOrigin(), false)
			end
		end
