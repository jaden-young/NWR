...
if not Music then music = class({}) end

function Music:PlayGameMusic(gameState)
	local PlayerCount = PlayerResource:GetPlayerCount() - 1

	if  gameState == 2 then
		Music:PlayGameMusicForState2(PlayerCount)
	end

	if  gameState == 3 then
		Music:PlayGameMusicForState3(PlayerCount);
	end

	if  gameState == 7 then
		Music:PlayGameMusicForState7(PlayerCount)
	end
end
...
function Music:PlayGameMusicForState2(PlayerCount)
...
function Music:PlayGameMusicForState3(PlayerCount)
	for i=0, PlayerCount do
		if PlayerResource:IsValidPlayer(i) then
			local player = PlayerResource:GetPlayer(i)
...
function Music:PlayGameMusicForState7(PlayerCount)
	for i=0, PlayerCount do
		if PlayerResource:IsValidPlayer(i) then
			local player = PlayerResource:GetPlayer(i)
...
function Music:PlayKillSound(killer, killed)
