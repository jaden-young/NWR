...
LinkLuaModifier("modifier_madara_burning_tree_dot", "abilities/heroes/madara/wood_release", LUA_MODIFIER_MOTION_NONE)

function madara_wood_release:Precache( context )
    PrecacheResource( "soundfile", "soundevents/game_sounds_heroes/game_sounds_kunkka.vsndevts", context )
    PrecacheResource( "soundfile", "soundevents/heroes/madara/madara_trees.vsndevts", context )
...
function madara_wood_release:OnAbilityPhaseStart()
	self:GetCaster():EmitSound("madara_wood_release_cast")
	return true
end
...
function madara_wood_release:BurnTree(tree)
	...
	Timers:CreateTimer( tree_burn_duration, function()
			...
			local particledestroy = ParticleManager:CreateParticle("particles/units/heroes/madara/tree_destroy_madara.vpcf", PATTACH_CUSTOMORIGIN, nil) 
			ParticleManager:SetParticleControl(particledestroy , 0, origin)
		return nil
	end)
end
